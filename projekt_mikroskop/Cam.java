...
public class Cam extends Application {
    ...
    double zoom = 1.0;
    double contrast = 1.0;//saturation
    ...
    public void start(Stage primaryStage) {
        ...
        root.setStyle("-fx-background-color: #93c5fd;"); // jasny zielony: #ccffcc
        ...
        // Animacja odświeżania obrazu
/*
        timeline = new Timeline(new KeyFrame(Duration.millis(130), e -> disp_frame()));
        timeline.setCycleCount(Timeline.INDEFINITE);
        timeline.play();
*/
        Timeline timeline = new Timeline(new KeyFrame(Duration.millis(100), e -> drawFrame()));/////?
        timeline.setCycleCount(Timeline.INDEFINITE);
        timeline.play();
    ...
    private void adjustContrast(double delta) {
       /* saturation = Math.max(0.5, Math.min(2.0, saturation + delta));
        drawFrame();*/
    ...
    private void snap(Stage primaryStage) {
        ...
        if (file != null) {
            try {
                ...
                System.err.println("Wystąpił błąd podczas zapisywania zdjecia: " + e.getMessage());
            }
        }
    }
}
